...
def map_salesperson_to_orm(d_obj: domain.Seller) -> models.Seller:
    return models.Seller(id=d_obj.id, username=d_obj.username)


def map_client_to_orm(d_obj: domain.Client) -> models.Client:
    return models.Client(id=d_obj.id, username=d_obj.username)
...
def map_seller_to_domain(orm_obj: models.Seller) -> domain.Seller:
    return domain.Seller(
        seller_id=orm_obj.id, username=orm_obj.username, rating=orm_obj.rating, is_active=True
    )
...
def map_product_to_domain(orm_obj: models.Product) -> domain.Product:
    calculated_quantity = (
        0
        if "stock" in inspect(orm_obj).unloaded
        else len([unit for unit in orm_obj.stock if not unit.is_sold])
    )
