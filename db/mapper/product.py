...
def map_product_to_domain(orm_obj: models.Product) -> domain.Product:
    ...
    if "variants" not in insp.unloaded:
        variants = [
            map_product_variant_to_domain(variant) for variant in orm_obj.variants
        ]
    category = None
...
def map_product_to_orm(d_obj: domain.Product) -> models.Product:
    return models.Product(
        ...
        variants=(
            [map_product_variant_to_orm(variant) for variant in d_obj._variants]
            if d_obj._variants
            else []
        ),
    )
...
def map_product_variant_to_domain(
    orm_obj: models.ProductVariant,
...
registry.register(
    ...
    to_orm=map_product_variant_to_orm,
    to_domain=map_product_variant_to_domain,
)
