<template>
  <div class="min-h-screen bg-gray-50 font-sans text-gray-900">
    <main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="flex items-center space-x-8 border-b border-gray-200 mb-8">
        <button
          @click="activeTab = 'products'"
          :class="['pb-3 font-medium text-lg transition-colors border-b-2', activeTab === 'products' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700']"
        >
          Мои товары
        </button>
        <button
          @click="activeTab = 'sales'"
          :class="['pb-3 font-medium text-lg transition-colors border-b-2 flex items-center', activeTab === 'sales' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700']"
        >
          Мои продажи
          <span v-if="sales.length" :class="['ml-2 text-xs px-2 py-0.5 rounded-full', activeTab === 'sales' ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-200 text-gray-600']">
            {{ sales.length }}
          </span>
        </button>
      </div>
            ...
            <table v-else class="w-full text-left border-collapse">
              ...
              <tbody class="divide-y">
                <tr v-for="product in products" :key="product.id" class="hover:bg-gray-50 transition">
                  ...
                  <td class="p-4 text-gray-600">{{ product.variants_count || 0 }} опций</td>
                  <td class="p-4">
                    <span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full font-medium">
                      {{ product.items_count || 0 }} шт.
                    </span>
                  </td>
      ...
      <div v-else-if="activeTab === 'sales'" class="space-y-6 animate-fadeIn">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">История продаж</h1>

        <div class="bg-white rounded-xl border shadow-sm overflow-hidden">
          <div v-if="sales.length === 0" class="p-12 text-center text-gray-500">
            <UIcon name="i-heroicons-shopping-bag" class="w-16 h-16 mx-auto text-gray-300 mb-3" />
            У вас пока нет продаж. Как только кто-то купит ваш товар, сделка появится здесь.
          </div>

          <table v-else class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-gray-50 border-b text-sm text-gray-500">
                <th class="p-4 font-medium">Номер заказа</th>
                <th class="p-4 font-medium">Сумма</th>
                <th class="p-4 font-medium">Статус</th>
                <th class="p-4 font-medium text-right">Чат покупателя</th>
              </tr>
            </thead>
            <tbody class="divide-y">
...
// Состояния вкладок
const activeTab = ref('products') // 'products' или 'sales'
const currentView = ref('list') // Для товаров: 'list' или 'create'

// Данные
const products = ref([])
const sales = ref([]) // <-- Новый массив для продаж
...
const fetchSales = async () => {
  try {
    // Изменил URL на твой новый эндпоинт
    const response = await $api('/api/seller/sales')
    // На всякий случай сортируем, чтобы новые сделки были сверху
    sales.value = response.sort((a, b) => b.id - a.id)
  } catch (error) {
    toast.add({ title: 'Ошибка', description: 'Не удалось загрузить продажи', color: 'red' })
  }
}
...
onMounted(() => {
  fetchProducts()
  fetchCategories()
  fetchSales() // <-- Добавили вызов при загрузке страницы
})
...
const addVariant = () => {
  form.value.variants.push({
    price: null,
    systemAttributes: buildDefaultSystemAttributes(),
    customAttributes: [{ key: '', value: '' }],
    rawKeys: ''
  })
}
