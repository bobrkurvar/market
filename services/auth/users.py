...
from domain import (CredentialsValidateError, MissingRoleError, Seller,
                    User, UserLoginNotFoundError, UserRole, Admin)
...
def get_user_from_payload(payload: dict) -> CurrentUser:
...
async def get_admin_from_user(user: CurrentUser, uow):
    if user.role != UserRole.admin:
        raise MissingRoleError(user_id=user.id, role=user.role)

    async with uow:
        return await uow.db.read_one(Admin, id=user.id, with_raise=True)


async def get_seller_from_user(user: CurrentUser, uow) -> Seller:
