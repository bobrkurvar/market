...
from domain import Client, Order, OrderStatuses, Product, ProductItem
...
def test_create_order_obj_with_client_id_and_product_id():
    order = Order(client_id=1, product_id=1, price=1)
...
def test_create_order_obj_with_client_and_product():
    order = Order(
        ...
        price=1,
        product_id=1,
        product=Product(title="title", price=7, product_id=2, seller_id=1),
        client=Client(username="username", client_id=2),
    )
    assert order.client_id == 2 and order.product_id == 2


def test_create_order_obj_with_new_client():
    product = Product(title="title", price=7, product_id=2, seller_id=1)
    client = Client(username="username")
    order = Order(client_id=1, product_id=1, product=product, client=client, price=1)
    assert order.client_id is None and order.product_id == 2
...
def test_create_order_obj_with_items():
    with pytest.raises(
        ...
        Order(
            ...
            product_id=1,
            price=1,
            items=ProductItem(content="content", product_id=1),
        )
...
def test_cancel_order_fail_with_empty_list_items():
    order = Order(client_id=1, product_id=1, items=[], price=1)
...
def test_cancel_paid_order():
    ...
    order = Order(order_id=1, client_id=1, product_id=1, items=[], price=1)
...
def test_cancel_order_success():
    item = ProductItem(content="content", product_id=1)
    order = Order(order_id=1, client_id=1, product_id=1, items=item, price=1)
    order.cancel()
